<?php // 92 > Statistiques de visites ;
...
function poin($dir){
	...
	if(is_dir($dir)){
		$diro=dir($dir);
		while ($entry=$diro->read()){
			  if($entry!="." && $entry!=".." && $entry!="stats"){
				if(is_file("$dir/$entry")){
					$statentry = stat("$dir/$entry");				
					$poids+=$statentry[7];
					//echo"\n<!-- $dir/$entry / $statentry[7] octets-->";
				}
				if(filetype("$dir/$entry") == "dir"){
					$poids += poin("$dir/$entry");
				}
			  }
		 }
	 }
 ...
 if(!isset($_SESSION['ses_totalpoi'])){
 	$ses_totalpoi= poin("..");
	$_SESSION['ses_totalpoi'] = $ses_totalpoi;
 }
 $totalpoi = $_SESSION['ses_totalpoi'];
 $autorised = round(2000000000/$totalpoi,2)*3;
 
 $totalpoids = $totalpoi;
 				if($totalpoi >1000000){
					$totalpoi/=1000000;
					$totalpoi=round($totalpoi,2);
					$totalpoi.=" Mo";
				}
				else{
					$totalpoi/=1000;
 ...
 echo"<b>$totalpoi</b><br>";
 //traffic recommandé: $autorised visites mensuelles<br>";
 */
///////////////////////////////////////////////:class total
$dir = dir("$logs/");
$toutotal=0;
...
$dir = dir("$logs/");
 ...
 <br><img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' width='400' height='1'><br>
 ...
 while ($entry=$dir->read()){
	  if((ereg('unik',$entry))){
		 ...
		 if($cetotal == $annee){
		 	...
		 	echo"<td><a href='index.php?annee=$cetotal'>$cetotal</a></td>";
		 }
		 
	 }
 }  
 echo"</tr></table><img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' width='400' height='1'><br>";
...
if($act == 'date'){
	echo"
	<br><img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' width='400' height='1'><br>
	[-]-<a href='index.php?annee=$annee'><b>classement par date</b></a>:
	...
	$dir = dir("$logs/");
	 ...
	 for($d=1 ; $d<$nbar ; $d++){
	  ...
	  $tlj = '';
	   ...
	   $cemoiiii = $titre_moi;
	   echo"
	   <font color='999999'>&nbsp;&nbsp;[-]--</font><a href='index.php?annee=$annee&act=date'><font class=\textefonce'><b>$titre_moi</b></font></a> ($nb_dat)<br>";
		 for($i=1 ; $i<=$nb_dat-1 ; $i++){
				...
				$tlja[$jour] = $nbcejour;
				 $tlj.='!'.$jour;
				  if($jour != $keljour){					   
					   ...
					   <a href='index.php?annee=$annee&act=date&keldate=$cettedate&keljour=$jour'><font class='textefonce'>$jour</font></a> $nbcejour<br>";
				  }
				  if($jour == $keljour){ 			
					    echo"
					   <font color='AAAAAA'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[- ]-----</font>
					   <a href='index.php?annee=$annee&act=date&keldate=$cettedate'><b><font class='textefonce'>$jour</font></b></a> ($nbcejour)<br>";
						$nbhpj = $nbcejour;
						for($h=0 ; $h<23 ; $h++){
							$he=$h;
							if($he<10) $he="0$he";
		   ...
		   echo"
		   <font color='999999'>&nbsp;&nbsp;[+]--</font><a href='index.php?annee=$annee&act=date&keldate=$cettedate'><font class=\textefonce'>$titre_moi ($nb_dat)</font></a><br>";
	   }
			...
			$testop = $tlh; 
			 ...
			 <tr><td valign='top' align='right' bgcolor='#DDDDDD' style='height:20px'><font color='000000' style='font-size:9px'><b>$top</b>_<br></td>";
				  for($h=0 ; $h<=23 ; $h++){
					...
					<br>
					<img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' width='20' height='$empty'><br><img src='http://www.adeli.wac.fr/vers/$vers/$theme/red.gif' width='20' height='$nbres'><br>
 ...
 echo"<br><img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' width='400' height='1'><br>[+]-<a href='index.php?annee=$annee&act=date'><b>classement par date</b></a>:";
}
echo"<font size='2'>";
///////////////////////////////////////////////////:prov
if($act == 'prov'){
		echo"<br><img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' width='400' height='1'><br>[-]-<a href='index.php?annee=$annee'><b>provenance</b></a><font size='1'>";
	...
	if(isset($_GET['dev'])){
		echo"(<a href='index.php?annee=$annee&act=prov&keldate=$cetotal'>liens généralisés</b></a>)";
	}
	else{
		echo"(<a href='index.php?annee=$annee&act=prov&keldate=$cetotal&dev='>liens complets</b></a>)";
	}
	if(!isset($_GET['cla'])){
		echo"(<a href='index.php?annee=$annee&act=prov&keldate=$cetotal&dev=&cla='>tous par ordre alphabétique</b></a>)";
	}
	else{
		echo"(<a href='index.php?annee=$annee&act=prov&keldate=$cetotal&dev='>ordre de fréquentation</b></a>)";
	}
	echo"
	...
	for($e=0 ; $e<$nbar ; $e++){
		...
		$empty = 100-$nbres;
		 echo"<tr><td><font size='1'>$print</font></td><td><font size='1'>$nbres%</td><td><img src='http://www.adeli.wac.fr/vers/$vers/$theme/red.gif' height='5' width='$nbres'><img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' height='5' width='$empty'></td></tr>";
	}
	 echo"</table>";
}
else{
 echo"<br><img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' width='400' height='1'><br>[+]-<a href='index.php?annee=$annee&act=prov'><b>provenance</b></a>:";
}
///////////////////////////////////////////////////:mots clefs
if($act == 'key'){
		echo"<br><img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' width='400' height='1'><br>[-]-<a href='index.php?annee=$annee'><b>mots clefs</b></a><font size='1'>";
		if(!isset($_GET['cla'])){
			echo"(<a href='index.php?annee=$annee&act=key&keldate=$cetotal&dev=&cla='>tous par ordre alphabétique</b></a>)";
		}
		else{
			echo"(<a href='index.php?annee=$annee&act=key&keldate=$cetotal&dev='>ordre de fréquentation</b></a>)";
		}
		echo"
		</font><table><tr bgcolor='CCCCCC'><td><font size='1'>chemin</td><td colspan='2'><font size='1'>taux</td></tr>";
	
	 $fp_nav = fopen("$logs/find_$annee.log","r");
	 $val_nav = fread($fp_nav,filesize("$logs/find_$annee.log"));
	 $results='';
	 $nbnavtot=0;	
	 $val_nav = split("<>",$val_nav);
 ...
 echo"<br><img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' width='400' height='1'><br>[+]-<a href='index.php?annee=$annee&act=key'><b>mots clefs</b></a>:";
}
...
if($act == 'nav'){
	echo"<br><img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' width='400' height='1'><br>[-]-<a href='index.php?annee=$annee'><b>navigateurs</b></a>";

 $fp_nav = fopen("$logs/nav_$annee.log","r");
 $val_nav = fread($fp_nav,filesize("$logs/nav_$annee.log"));
 ...
 if(ereg("MSIE",$val_nav)){
	...
	echo"\n
	<br><table><tr bgcolor='CCCCCC'><td><font size='1'>navigateurs</td><td colspan='2'><font size='1'>taux</td></tr>"; 
	...
	for($e=0 ; $e<$nbara ; $e++){
		...
		$nbres=round(($artosnumb*100/$allresults),2);
		$empty=100-$nbres;
		 echo"<tr><td><font size='1'>$artosid</font></td><td><font size='1'>$nbres%</td><td><img src='http://www.adeli.wac.fr/vers/$vers/$theme/red.gif' height='5' width='$nbres'><img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' height='5' width='$empty'></td></tr>";
	}
	 echo"</table>";
	// */
 ...
 echo"<br><img src='http://www.adeli.wac.fr/vers/$vers/$theme/gray.gif' width='400' height='1'><br>[+]-<a href='index.php?annee=$annee&act=nav'><b>navigateurs</b></a>:";
}
