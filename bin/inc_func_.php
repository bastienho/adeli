<?php // 315 > Bibliothèque de fonctions ;
/******************************                              M Y S Q L                               **********************************/
...
function get($base,$champ,$id,$from='id'){
	$rus = mysql_query("SELECT `$champ` FROM `$base` WHERE `$from`='$id'");
...
function updatedb($base,$db,$id,$sup="",$function=''){
   ...
   $videcom = str_replace("'","",$command);
   $videcom = trim(str_replace(",","",$videcom));
   //echo"<!-- UPDATE `$db` SET $command WHERE `id`='$id' -->";
   if($videcom != ""){
	   $command = substr($command,0,strlen($command)-2);
...
function addfile($chemin,$dest,$file,$dangerous){	
	$imacool = array("jpg","jpeg","gif","png","bmp","swf");
	$ext = strtolower(substr(strrchr($dest,"."),1));
	$chemin = ereg_replace("//","/",$chemin);
	...
	elseif(is_dir($chemin) || $finalext=="dir"){
		...
		if($finalext=="dir"){
			$chemin = ereg_replace(".dir","",$chemin);
		}
	echo"<!-- FILE $finalext -->";
		$ex="";	
...
/////////////////////////////////////////////////////////////////////// RENAME FILE
	...
	if(is_file("$path_ren/$file_ren")){
		if(rename("$path_ren/$file_ren","$path_ren/$nna")){
			...
			$return.=returnn("renommage effectuée avec succès","009900",$vers,$theme);
		}
		else{
			$return.=returnn("le renommage a échouée<br>$file_ren > $nna","990000",$vers,$theme);
		}
	}
	else{
			$return.=returnn("le fichier spécifié n'existe pas ($file_ren)","990000",$vers,$theme);
		}

}

$js_url_replace="replace(/[\s]/g,'-').replace(/[,;\`|[)([~\\!']/g,'').toLowerCase().replace('é','e').replace('à','a').replace('ï','i').replace('ù','u').replace('ô','o')";
$js_www_replace="replace(/[\s]/g,'-').replace(/[,;:\`|[)($[=~&\\?\\/!']/g,'').toLowerCase().replace('é','e').replace('à','a').replace('ï','i').replace('ù','u').replace('ô','o')";
...
function geticon($pathe,$tid,$alt,$style=''){   //////////////////////////// GET ICONE
	$imacool = array('jpg','jpeg','gif','png','bmp','swf');
	$imaaa="";
		for($ic=0; $ic<sizeof($imacool) ; $ic++){
			if(is_file("../$pathe/$tid.$imacool[$ic]")){
				if($ic==5){
					$imaaa="<EMBED quality=high scale='noscale' src='./?incpath=_ima.php&file=$pathe/$tid.$imacool[$ic]' TYPE='application/x-shockwave-flash' PLUGINSPAGE='http://www.macromedia.com/go/getflashplayer'  $style></EMBED>";	
				}
				else{				
					$imaaa="<img src=\"./?incpath=_ima.php&file=$pathe/$tid.$imacool[$ic]\" alt=\"$alt\" $style>";	
					}
				break;
			}
		}
...
function urluntranslate($url){
	...
	$url = ereg_replace("[eéêèÊÈÉE]","[eéêèÊÈÉE]+",$url);
	$url = ereg_replace("[aàâÂÀA]","[aàâÂÀA]+",$url);
	$url = ereg_replace("[uùûÛÙU]","[uùûÛÙU]+",$url);
	$url = ereg_replace("[oôÔO]","[oôÔO]+",$url);
	$url = ereg_replace("[iîïÎÏÌI]","[iîïÎÏÌI]+",$url);
	$url = ereg_replace("[cçC]","[cçC]+",$url);
	foreach($csnm as $k=>$l){
...
function geturl($url){
	...
	$url = str_replace(' ','',$url);
	$url = ereg_replace("[eéêèÊÈÉ]","e",$url);
...
function html_my_text($txt){
	if(!ereg("</",$txt) && !ereg("<br />",$txt) ){
		$taxt = split("[ \n]",$txt);
		$toxt="";
...
function editor($field_name='texteHTML',$field_value='',$i='',$stylo='',$edition=1, $editchange=1){
		global $vers,$prov,$part, $edit_size_site, $edit_font_site;
		if($edition==1){
			if(is_file("../style.css")){
				$field_value="<head><link rel='stylesheet' type='text/css' href='http://$prov/style.css'/></head><body>$field_value</body>";
			}
			else{
				$field_value="<head><link rel='stylesheet' type='text/css' href='http://$prov/$part/style.css'/></head><body>$field_value</body>";
			}
		}
		elseif($editchange==1 && $field_value==''){
			$field_value="<body></body>";
		}
		echo" <table border='0' cellpadding='0' width='450' cellspacing='0' style='margin-top:5px'>
			  <tr>
			<td>";
		if($edition==1 || $editchange==1 ){
				echo"
				
	  <table cellpadding='0' cellspacing='0' border='0' bgcolor='FFFFFF' id='edit_tools_$i' style='display:block'>
<tr valign=\"middle\" class='buttontd'> 
			  				<td class='editor_td'>
							
									<img onclick=\"document.getElementById('edit_font_$i').style.display='block';\" src=\"http://www.adeli.wac.fr/vers/$vers/images/police.gif\" alt=\"Police\">
									<div id='edit_font_$i' class='buttontd'>";
									foreach($edit_font_site as $fonti){
										echo"<a onclick=\"editbox_$i.document.execCommand('fontname', false, '$fonti'); editbox.focus();\"  style=\"font-family:$fonti\">$fonti</a><br>";	
									}
									echo"<hr>
										...
										<img onclick=\"document.getElementById('edit_size_$i').style.display='block';\" src=\"http://www.adeli.wac.fr/vers/$vers/images/size.gif\" alt=\"Taille\">
										<div id='edit_size_$i' class='buttontd'>
										";
									foreach($edit_size_site as $sizei){
										echo"<a onClick=\"taille($i,'$sizei',false)\">$sizei</a><br>";	
									}
									echo"<hr>
										...
										<img 
										onclick=\"editbox_$i.document.execCommand('bold', false, null); editbox_$i.focus();\" src=\"http://www.adeli.wac.fr/vers/$vers/images/bold.gif\" alt=\"gras\"><img 
										onClick=\"editbox_$i.document.execCommand('italic', false, null); editbox_$i.focus();\" src=\"http://www.adeli.wac.fr/vers/$vers/images/italic.gif\" alt=\"italic\"><img 
										onClick=\"editbox_$i.document.execCommand('underline', false, null); editbox_$i.focus();\" src=\"http://www.adeli.wac.fr/vers/$vers/images/underline.gif\" alt=\"souligne\"><img 
										onClick=\"editbox_$i.document.execCommand('StrikeThrough', false, null); editbox_$i.focus();\" src=\"http://www.adeli.wac.fr/vers/$vers/images/strike.gif\" alt=\"barre\">
										
										</td><td class='editor_td'>
										
										<img onclick=\"document.getElementById('edit_align_$i').style.display='block';\" src=\"http://www.adeli.wac.fr/vers/$vers/images/left.gif\" alt=\"alignement\">
										<div id='edit_align_$i' class='buttontd'>
													<a onClick=\"editbox_$i.document.execCommand('justifyleft', false, null); editbox_$i.focus();\" ><img src=\"http://www.adeli.wac.fr/vers/$vers/images/left.gif\" align='absmiddle' alt=\"gauche\">Aligner à gauche</a><br>
													<a onClick=\"editbox_$i.document.execCommand('justifycenter', false, null); editbox_$i.focus();\"><img  src=\"http://www.adeli.wac.fr/vers/$vers/images/center.gif\" align='absmiddle' alt=\"centrer\">Centrer</a><br>
													<a onClick=\"editbox_$i.document.execCommand('justifyright', false, null); editbox_$i.focus();\" ><img src=\"http://www.adeli.wac.fr/vers/$vers/images/right.gif\" align='absmiddle' alt=\"droite\">Aligner à droite</a><hr>
													<a onClick=\"justify($i,'right');\"><img  src=\"http://www.adeli.wac.fr/vers/$vers/images/justify.gif\" align='absmiddle' alt=\"justifier\">Justifier</a><br>
													<a  onClick=\"float($i,'right');\"><img  src=\"http://www.adeli.wac.fr/vers/$vers/images/floatright.gif\" align='absmiddle' alt=\"flottant à droite\">Flottant à droite</a><br>
													<a  onClick=\"float($i,'left');\"><img  src=\"http://www.adeli.wac.fr/vers/$vers/images/floatleft.gif\" align='absmiddle' alt=\"flottant à gauche\">Flottant à gauche</a><hr>
													<a onClick=\"editbox_$i.document.execCommand('InsertUnorderedList', false, null); editbox_$i.focus();\"><img  src=\"http://www.adeli.wac.fr/vers/$vers/images/list.gif\" align='absmiddle' alt=\"liste\">Liste à puce</a><br>
													<a onClick=\"editbox_$i.document.execCommand('InsertOrderedList', false, null); editbox_$i.focus();\"><img  src=\"http://www.adeli.wac.fr/vers/$vers/images/list.gif\" align='absmiddle' alt=\"liste\">Liste numérotée</a><br>
													<a onClick=\"editbox_$i.document.execCommand('Indent', false, null); editbox_$i.focus();\" ><img src=\"http://www.adeli.wac.fr/vers/$vers/images/left.gif\" align='absmiddle' alt=\"liste\">Augmenter le retrait</a><br>
													<a onClick=\"editbox_$i.document.execCommand('Outdent', false, null); editbox_$i.focus();\" ><img src=\"http://www.adeli.wac.fr/vers/$vers/images/left.gif\" align='absmiddle'  alt=\"liste\">Diminuer le retrait</a><hr>
													<a onclick=\"document.getElementById('edit_align_$i').style.display='none';\">Fermer</a>
										</div>
										</td><td class='editor_td'>
										
										<img src='http://www.adeli.wac.fr/vers/$vers/images/fgcolor.gif' alt='couleur de texte' onclick=\"choosecolor($i,'ForeColor','$field_name','html',event)\"><img src='http://www.adeli.wac.fr/vers/$vers/images/bgcolor.gif' alt='couleur de fond' onclick=\"choosecolor($i,'Backcolor','$field_name','html',event)\">
										</td><td class='editor_td'>
										<img onclick=\"document.getElementById('edit_links_$i').style.display='block';\" src=\"http://www.adeli.wac.fr/vers/$vers/images/link.gif\" alt=\"lien\">
										<div id='edit_links_$i' class='buttontd'>
													<a onClick=\"addlink($i)\"><img src=\"http://www.adeli.wac.fr/vers/$vers/images/link.gif\" align='absmiddle' alt=\"lien hypertexte\"> Lien hypertexte</a><br>
													<a  onClick=\"editbox_$i.document.execCommand('Unlink',false,null); editbox_$i.focus();\"><img src=\"http://www.adeli.wac.fr/vers/$vers/images/unlink.gif\" align='absmiddle' alt=\"enlever les liens\"> Supprimer le lien</a><br>
													<a  onClick=\"addancre($i)\"><img src=\"http://www.adeli.wac.fr/vers/$vers/images/ancre.gif\" align='absmiddle' alt=\"ancre\"> Ajouter une ancre</a>
													<hr>
													<a onclick=\"document.getElementById('edit_links_$i').style.display='none';\">Fermer</a>
										</div>
										</td><td class='editor_td'><img 
										onClick=\"editbox_$i.document.execCommand('InsertHorizontalRule', false, null); editbox_$i.focus();\" src=\"http://www.adeli.wac.fr/vers/$vers/images/line.gif\" alt=\"ligne\">
										<img  
										onClick=\"sautdeligne($i)\" src=\"http://www.adeli.wac.fr/vers/$vers/images/br.gif\" alt=\"saut de ligne\">
										<img   onClick=\"document.getElementById('tableau_$i').style.display='block';\" src=\"http://www.adeli.wac.fr/vers/$vers/images/table.gif\" alt=\"tableau\">		
										<div id='tableau_$i' class='buttontd'>
												<b>Insérer un tableau</b><hr>
												 ...
												 <tr><td>Espacement : </td><td><input type='text' value='0' onkeyup='javascript:tab_s_$i=this.value' size='2'>px</td></tr>
												  <tr><td>marge intérieure : </td><td><input type='text' value='3' onkeyup='javascript:tab_p_$i=this.value' size='2'>px</td></tr>
												  ...
												  <input type='button' value='insérer' class='buttontd' onclick=\"tableau($i,tab_x_$i,tab_y_$i,tab_b_$i,tab_c_$i,tab_s_$i,tab_p_$i);document.getElementById('tableau_$i').style.display='none';\">
												  </td></tr>
												  </table>
										</div>
												
		 								 </td><td class='editor_td'>
										 <img onclick=\"document.getElementById('edit_edition_$i').style.display='block';\" src=\"http://www.adeli.wac.fr/vers/$vers/images/code.gif\" alt=\"code\">
										<div id='edit_edition_$i' class='buttontd'>
													 <a onClick=\"addcode($i)\"><img src=\"http://www.adeli.wac.fr/vers/$vers/images/code.gif\" align='absmiddle' alt=\"<html>\">insérer du code</a><br>
													 <a  onclick='fenp_$i()'><img src=\"http://www.adeli.wac.fr/vers/$vers/images/code.gif\" align='absmiddle' alt=\"-> <-\">Fenêtre réduite</a><br>
													 <a  onclick='feng_$i()'><img src=\"http://www.adeli.wac.fr/vers/$vers/images/code.gif\" align='absmiddle' alt=\"<- ->\">Fenêtre adaptée</a><br>
													 <a onclick=\"srcsty = document.getElementById('htmlsrc$i').style;if(srcsty.display=='none'){srcsty.height='200px';srcsty.display='block';document.fourmis.$field_name.focus;}else{srcsty.height='5px';srcsty.display='none';}\"><img src=\"http://www.adeli.wac.fr/vers/$vers/images/code.gif\" align='absmiddle' alt=\"<source>\">Source</a><br>
													<a onClick=\"format($i)\"><img src=\"http://www.adeli.wac.fr/vers/$vers/images/eraz.gif\" align='absmiddle' alt=\"formater le texte\">Formater</a>
														 <hr>
	  ...
	  <tr>
		<td>
		 ";
		 if($edition==1 || $editchange==1 ){
		  echo"<textarea name=\"$field_name\" id=\"htmlsrc$i\" style=\"width:450px;height:1px;display:none;margin-top:5px;\" onfocus='editionde$i=true' onblur='editionde$i=false'>$field_value</textarea>
		  <iframe width='450' height='250' src='about:blank' name='editbox_$i' id='editb_$i' style='height:250px;display:block'></iframe>";
		 }
		 else{
		  echo"<textarea name=\"$field_name\" id=\"htmlsrc$i\" style=\"width:450px;height:250px;display:block;margin-top:5px;\" onfocus='editionde$i=true' onblur='editionde$i=false'>$field_value</textarea>";
		 }
		echo"</td>
	  </tr>
	</table> 			 
	...
	function save_$i(){
		if(ise$i==1){
		   ...
		   if(isg$i){
			...
			document.getElementById('edit_simp_$i').style.display='none';
		}
		else{
			document.getElementById('edit_tools_$i').style.display='none';
			document.getElementById('editb_$i').style.display='none';
			document.getElementById('edit_simp_$i').style.display='block';
			document.getElementById('htmlsrc$i').style.display='block';
...
function returnn($return,$color){
	global $vers;
	global $theme;
	return "<table width='300' style='margin:1px' cellspacing='0' cellpadding='0' border='0' bgcolor='#$color'>
			<tr ><td align='left' valign='top'><img src='http://www.adeli.wac.fr/vers/$vers/$theme/corn_ctl.gif' alt='.'></td>
			<td align='center' valign='middle'></td>
			<td align='right' valign='top'><img src='http://www.adeli.wac.fr/vers/$vers/$theme/corn_ctr.gif' alt='.'></td></tr>
			<tr><td colspan='3' align='center' valign='middle'><font color='#FFFFFF' size='1'>$return</font></td></tr>
			<tr><td align='left' valign='bottom'><img src='http://www.adeli.wac.fr/vers/$vers/$theme/corn_cbl.gif' alt='.'></td>
			<td align='left' valign='bottom'></td>
			<td align='right' valign='bottom'><img src='http://www.adeli.wac.fr/vers/$vers/$theme/corn_cbr.gif' alt='.'></td></tr>
			</table>";
}
/////////////////////////////////////////// POIDS
...
if($menu){
	...
	//if(preg_match("!Firefox+!i",getenv("HTTP_USER_AGENT"))){
		$ret="<a href=\"#\" style=\"display:block;float:right\">$titre <font size='1' color='#EEEEEE'>IE6</font><span>
			...
			<div id=\"pickercode\" style=\"background-color:#FFFFFF;width:50px\">$field_value</span>
			</td><td valign=\"top\">
	...
	//$action = ereg_replace('COLOR','NONE',$actiona);
	$ret="<table class=\"buttontd\" style=\"position:absolute;top:0px;left:0px;\"><tr><td valign=\"top\">
								<div id=\"divo$field_name\" style=\"background-color:#$field_value;padding:3px;position:relative;height:70px;width:50px;border-style:solid;border-color:#000000;border-width:1px\"></div><br>
								<div id=\"pickercode\" style=\"background-color:#FFFFFF;width:50px\">$field_value</div><br>
			...
			while($ru = mysql_fetch_array($rus)){
				...
				if($ty=='date' || $ty=='time' || $ty=='datetime'){
					...
					if(ereg('desc',strtolower($urle[2]))){
						...
						elseif($tim<strtotime()) $col='330000';
...
///////////////////////////////: COMPTA
$defstat = array("attente","validé","préparé","expédié","annulé");
