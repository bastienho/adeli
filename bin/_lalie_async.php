				   ...
				   if(!$rem || mysql_num_rows($rem)==0){
					   ...
					   for ($i = 0; $i < $columns; $i++) {
						...
						if(is_file('_lalie_desinsc.php')){
							$msg_html = stripslashes($mess)." <a href='http://$prov$path/_lalie_desinsc.php?r=".$eid."&m=".$email."&s=".$s."' target='_blank'><font color='999999' size='1'>se désinscrire</font></a> ";
						}
						else{
...
mail(\"$email\",\"$object\",\"$msg\",\"$headers\");
			...
			if( mysql_query("UPDATE `$lalierp` SET `active`=1 WHERE `id`='$rid'") ){
				echo"1 : la lettre a bien été envoyée ($totlml)";
			}
			else{
				echo"1 : la lettre a bien été envoyée, mais le rapport a échoué";
			}
			
		}
		elseif($object != "" && $de != "" && $u_email != "" && sizeof($listmail)<2 && $ro[1]!='' ){
			echo"1 : lettre déjà envoyée";
			if( mysql_query("UPDATE `$lalierp` SET `active`=1 WHERE `id`='$rid'") ){
				echo" ";
			}
			else{
				echo" mais le rapport a échoué";
			}
		}
		else{
