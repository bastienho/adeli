<?php // 620 > Gestion des articles ;

		$co = abs(get_pref("ga.four.conf"));
		 $visus = array('none','block');
		 $vis = $visus[$co];
		if(isset($_GET['fours'])){ 
			$vis='block'; 
			set_pref("ga.four.conf",1);
		}
		$visy='block';
		if($vis=='block') $visy='none';
		
		$ga_parts = array('colonel','vent','stoc','achafour','otherplus','imags');
		$ga_partn = array('Description','Vente','Stock','Achats','Plus','Images');
		...
		function inpar(ki){
			...
			foreach($ga_parts as $k=>$v){
				...
				document.getElementById('lien_$v').className='buttontd';
				";	
				if($k==$co){
					...
					$liens.="<a id='lien_$v' class='buttontd' style='cursor:pointer' onclick=\"inpar($k)\">$ga_partn[$k]</a>";
					${'vis'.$k}='none';	
				}
			...
			document.getElementById('lien_'+ga_parts[ki]).className='menuselected';
			ajaxfil('gafour',ki);
		}
		</script>
	...
	$fichiers["gestion_articles"] = $fich_temp;
	  ...
	  <script language='javascript' type='text/javascript'>
	 	function addinref(koi,addf,notforall){
			var fa='forall';
			if(notforall) fa='notforall';
			if(!addf) addf = prompt(\"Veuillez saisir le nom de votre nouvelle \"+koi+\"\\ndans le champs ci-dessous\",'');
			if(addf){
				document.fourmis.action='./?option=$option&part=$part&edit=$edit&update&'+fa+'&addref='+koi+'&val='+addf+'#_'+addf;
		...
		function ch_achat(li,val,ref){
			...
			//eval('document.fourmis.prix_pro'+ref+'.value=\"'+val+'\";'); 
		...
		function orf(type,li,ref){
			...
			eval('pac = document.fourmis.prix_rev'+ref+'.value;');
			eval('commi = parseFloat(document.fourmis.com'+ref+'.value)');
			pvb = parseFloat(pvb);
			pco = parseFloat(pco);
			pac = parseFloat(pac);
			...
			if(tcom==1){
				mv = Math.round((np-pac)*commi)/100;
			}
...
<h3 id='globalname'></h3>
<table width='100%'>
	<tr><td><u>focus</u> <img src='$style_url/images/star$art_nouveaute.gif' border='none' alt='Focus: $art_nouveaute'> ";
		if($u_droits == '' || $u_dgw == 1 ){
	  ...
	  echo" &nbsp; &nbsp;
	  <u>activé</u> <img src='$style_url/$theme/v$art_active.gif' border='none' alt='actif: $art_active'> ";
		if($u_droits == '' || $u_dgw == 1 ){
			echo"oui<input type=\"radio\" name=\"active\" value=\"1\" $actouno[$art_active]>
	  ...
	  <input class=\"grosbouton\" type=\"submit\" value=\"Enregistrer\">
	  </td>
	  </tr>
	 <tr> <td valign='top'>
	  ";
	...
	echo"<a href='./?gestion_rayons&edit=$art_rayon'> Consulter</a>";
	
	if($edit!=0){
		echo" &nbsp;
		Alias <a class='info'><font color='#FF0000'>/!\</font><span>Compatible avec les boutiques développées à partir de 2010</span></a>  &nbsp;
		";
		if($u_droits == '' || $u_dgw == 1 ){
			...
			if($res && mysql_num_rows($res)>0){
				while($rox=mysql_fetch_array($res)){
					...
					empil(0,0,$rox[1]);
					echo"</select><a onclick=\"document.fourmis.action='./?option=$option&part=$part&edit=$edit&supp_alias=$rox[0]&refresh=1';document.fourmis.submit();\"><img src=\"$style_url/$theme/trash.gif\" border='none' alt='supprimer'></a> ";	
				}
			}
	...
	echo"<br><br>
	 ...
	 <div class='buttontd' style='padding-bottom:0px;text-align:left;'>$liens</div>
	 <div class='cadrebas'>
...
";																												
	...
	echo"
	<div id='colonel'  style='position:relative;display:$vis0; border-width:2px;text-align:left;'>
	  ...
	  $ris = mysql_query("SELECT * FROM `$langue_db`");
		if($ris && mysql_num_rows($ris)>0){
			while($riw=mysql_fetch_object($ris)){
				...
				if($i==1){
					echo"<script>
					document.getElementById('globalname').innerHTML=\"$lng_nom_val\";
					</script>";	
				}
				echo"
				<tr><td colspan='2'><b>$lng_nom</b></td></tr>
				";
				if($u_droits == '' || $u_dgw == 1 ){
					echo"
					<tr><td>Nom:</td>
						<td><input type=\"texte\" name=\"trad_nom_$lng_code\" value=\"$lng_nom_val\" size=\"30\" style='width:270px;'";
						if($i==1)	echo"onchange=\"document.getElementById('globalname').innerHTML=this.value\"";
						echo"></td>
					</tr>
					<tr><td>Description:</td>
						<td>
					<textarea name='trad_des_$lng_code' class='editor' style='width:270px;height:50px;'>$lng_des_val</textarea></tr>
...
////////////////////////////////////////////////////////////////// IMAGE
	...
	echo"<div id='imags'  style='position:relative;display:$vis5; border-width:2px;text-align:left;'>
	<div class='cadre'>";
	if($edit!=""){
		$fich_temp = $fichiers["gestion_articles"];
...
/**************************************************************************************************************************** 
	 ...
	 $claspar = trim(get_pref("gestion.articles.classpar"));
	if(isset($_GET['clp']) && ($_GET['clp']=='tai' || $_GET['clp']=='col')){
	 	 $claspar=$_GET['clp'];
		 set_pref("gestion.articles.classpar",$claspar); 
	 }
...
///////////////////////////////////////////////////// LISTE LES FAMILLES (FINAL)	
	...
	$artstock_buffer=array();
	 ...
	 echo"
	 <div  id='vent' style='position:relative;display:$vis1;  border-width:2px; text-align:left;'>
		Vente
	 ...
	 <tr class='buttontd'>
		...
		<td>Référence</td>
		<td>Code barre</td>
		<td><a href='./?option=$option&part=$part&edit=$edit&clp=$getoth' name='chif'><u>".ucfirst($rang_names[$clasoth])."</u></a></td>
	...
	function conmulti(k){
		...
		nbsel=0;
		posch='';
		var allche = document.getElementById('vent').getElementsByTagName(\"input\");
		...
		if(nbsel>0){
			...
			<input type='checkbox' name='all$i$c' onclick=\"tout(document.getElementById('vent'),this)\">&nbsp;<a href='#suppall' name='suppall' onclick=\"conmulti('delete')\"><img src='$style_url/$theme/trash.gif' border='none' alt='supprimer'></a>&nbsp;<a href='#suppall' onclick=\"conmulti('active')\" class='info'><img src='$style_url/$theme/v1.gif' border='none' alt='v'><span>Activer les articles cochés</span></a>&nbsp;<a href='#suppall' onclick=\"conmulti('desactive')\" class='info'><img src='$style_url/$theme/v0.gif' border='none' alt='o'><span>Désactiver les articles cochés</span></a>
			";
		}
		else{
			echo".";
	 ...
	 for($i=0 ; $i<sizeof($allkoi) ; $i++){
		...
		$stocks_tr.="<tr><td colspan='2'><b>$ligne</b></td></tr>";
		$artstock_buffer[]=$ligne;
	 ...
	 echo"<td align='right'>
		...
		$res = mysql_query("SELECT * FROM `gestion_artstock` WHERE `ref`='$edit' AND `$clasnom`='".str_replace("'","''",$ligne)."' ORDER BY `ordre`");
		 while($rou=mysql_fetch_object($res)){
				...
				if($ref && mysql_num_rows($ref)>0){
					while($fo=mysql_fetch_array($ref)){
						...
						if($li==0){
							...
							$fourtruc.="<td align='right' valign='middle' rowspan=$numref'><input type='text' name='prix_rev$s_id' value='$s_prix_rev' class='gaf_prix' onkeyup='ch_achat($fo[0],document.fourmis.prixpa_$fo[0].value,$s_id)'>&euro;</td>";
						}
						$pach = $s_prix_rev;
						$fourtruc.="
						<td>
						...
						";							
							if($li==0){
								$fourtruc.="<td valign='middle' rowspan=$numref'><input type='text' name='com$s_id' value='$s_com' class='gaf_txt' onkeyup='ch_achat($fo[0],document.fourmis.prixpa_$fo[0].value,$s_id)'>%
								<select name='comw$s_id' onchange='ch_achat($fo[0],document.fourmis.prixpa_$fo[0].value,$s_id)'>";
						...
						if($u_droits == '' || $u_active == 1 ){
							if($li==0){
								...
								<table id='org$fo[0]' class='cadre' cellspacing='0'>
									...
									<tr><td>Part.</td><td><input type='text' name='orpa$fo[0]' value='0.00' onkeyup=\"orf('pa',$fo[0],$s_id)\">%</td><td><span id='orpac$fo[0]'>$s_prix</span>&euro;</td><td><span id='orpav$fo[0]'>".round($s_prix*$s_com/100,2)."</span>&euro;</td><td><span id='orpam$fo[0]'>".($pvpa-$pach)."</span>&euro;</td><td>
									<a onclick=\"defi('pa',$fo[0],$s_id)\">Définitif</a> /
									<a onclick=\"prom('pa',$fo[0],$s_id)\">Promo</a>
									</td><td></td></tr>
									<tr><td>Pro.</td><td><input type='text' name='orpro$fo[0]' value='0.00' onkeyup=\"orf('pro',$fo[0],$s_id)\">%</td><td><span id='orproc$fo[0]'>$s_prix_pro</span>&euro;</td><td><span id='orprov$fo[0]'>".round($s_prix_pro*$s_com/100,2)."</span>&euro;</td><td><span id='orprom$fo[0]'>".($pvpro-$pach)."</span>&euro;</td><td>
			...
			$stocks_tr.="<tr><td>$infosprod</td><td>";
			/*if($u_droits == '' || $u_dgw == 1 ){
				//echo"<input type='text' name='stock$s_id' value='$s_stock' size='2'>";
			}
			else{
				$stocks_tr.="$s_stock";
			}*/
			$stocks_tr.="</td></tr>";
			$artstock_buffer[]=array($s_id,$infosprod);
	 ...
	 <a onclick=\"document.getElementById('newtai').style.visibility='visible';\" class='buttontd'>Ajouter ".$rang_names['taille']."</a>
		...
		<b>".ucfirst($rang_names['taille'])." : </b>
			...
			<option value=''></option>
				...
				if(isset($gestion_articles_taille_liste)){
					...
					$res = mysql_query("SELECT DISTINCT `taille` FROM `gestion_artstock` ORDER BY `taille`");
				 if($res && mysql_num_rows($res)>0){
					while($rou=mysql_fetch_array($res)){
		...
		<b>".ucfirst($rang_names['couleur'])." : </b>
			...
			<option value=''></option>
				...
				$res = mysql_query("SELECT DISTINCT `couleur` FROM `gestion_artstock` ORDER BY `couleur`");
				 if($res && mysql_num_rows($res)>0){
					while($rou=mysql_fetch_array($res)){
						echo"<option value=\"$rou[0]\">".ucfirst($rou[0])."</option>";
					}
...
<div id='stoc' class='buttontd' style='position:relative;display:$vis2; border-width:2px;text-align:left;'>
	<div class='cadre'>";
if(isset($shop_db) && is_array($shop_db) && sizeof($shop_db)==3){			
		
	if(!mysql_query("SHOW COLUMNS FROM gestion_multistock")  ){
			if(mysql_query("CREATE TABLE `gestion_multistock` (
	  `id` bigint(20) NOT NULL auto_increment,
	  `article` bigint(20) NOT NULL default '0',
	  `magasin` bigint(20) NOT NULL default '0',
	  `stock` int(4) NOT NULL default '0',
	  PRIMARY KEY  (`id`)
	)") ){
				$return.=returnn("La table <b>\"Stocks\"</b> a &eacute;t&eacute; cr&eacute;&eacute;e correctement","009900",$vers,$theme);
			}
			else{
				$return.=returnn("La table Stocks n'a pu être cr&eacute;&eacute;e correctement","990000",$vers,$theme);
			}
	}
}
echo"<table class='cadre'><tr><td colspan='2' class='buttontd'>Article</td>";
if(mysql_query("SHOW COLUMNS FROM gestion_multistock") && isset($shop_db) && is_array($shop_db) && sizeof($shop_db)==3 ){
	...
	if($res && mysql_num_rows($res)>0){
		while($roy=mysql_fetch_array($res)){
			...
			echo"<td class='buttontd'>$roy[1]</td>";
		}
	}
	echo"</tr>";
	$nbshop = mysql_num_rows($res)+2;
	foreach ($artstock_buffer as $artbuf){
		if(is_string($artbuf)){
			echo"<tr><td colspan='2'><b>$artbuf</b></td><td colspan='$nbshop'></td></tr>";
		}
		elseif(is_array($artbuf)){
			echo"<tr><td colspan='2'>$artbuf[1]</td>";
			foreach($shop_list as $shop_id=>$shop_nom){
				$resy = mysql_query("SELECT `id`,`stock` FROM `gestion_multistock` WHERE `article`='$artbuf[0]' AND `magasin`='$shop_id'");
				if($resy && mysql_num_rows($resy)>0){
					$roy=mysql_fetch_array($resy);
					if(isset($_POST["multistock_$roy[0]"])){
						if(mysql_query("UPDATE `gestion_multistock` SET `stock`='".$_POST["multistock_$roy[0]"]."' WHERE `id`='$roy[0]'")){
							$roy[1]=	$_POST["multistock_$roy[0]"];
						}
					}
				}
				else{
					mysql_query("INSERT `gestion_multistock` (`article`,`magasin`) VALUES ('$artbuf[0]','$shop_id')");
					$roy[1]=0;
					$roy[0]=mysql_insert_id($conn);
				}
				echo"<td><input type='text' value='$roy[1]' name='multistock_$roy[0]'/></td>";	
			}
			echo"</tr>";
		}
	}
...
echo"<div id='achafour' class='buttontd' style='position:relative;display:$vis3;  border-width:2px; text-align:left;'>
				
					 <span id='gaf_retour'></span>
	...
	$fourh+=30;
			echo" <table id='quantfour' style='background:#DDD ;position:absolute;visibility:hidden; top:35px; left:0px; width:895px;height:$fourh"."px;z-index:50;'><tr><td valign='middle' align='center'>
	...
	";
		...
		echo"<div id='stoc' class='buttontd' style='position:relative;display:$vis1;  border-width:2px; text-align:left;'>Enregistrez l'élément pour pouvoir gérer les stocks</div>
		<div id='achafour' class='buttontd' style='position:relative;display:$vis2;  border-width:2px; text-align:left;'>Enregistrez l'élément pour pouvoir gérer les stocks</div>"; 
	 }
/**************************************************************************************************************************** 
                                             A U T R E S                                                                      
                                                                                                                            */
//////////////////////////////////////////////////////////////////////// ASSOC
	 echo"
	 <div id='otherplus' class='buttontd' style='position:relative;display:$vis4;  border-width:2px; text-align:left;'>
			...
			/* ///////////////////////////////////// SUFIXE
			 ...
			 elseif( ( ereg("_",$field_act) && mysql_query("SHOW COLUMNS FROM `$mot`") ) || ereg('@',$field_act) ){
				...
				if(substr($field_act,0,1) == "@"){
					...
					$field_value = $_SESSION[$nameofoption];	
					echo"<td width='100'>$nameifthefield <a class='info'><img src='$style_url/$theme/pile.gif'>
					<span>Ce champ est rempli automatiquement</span></a></td><td>
					 <img src='$style_url/$theme/mysqltype-special.png' alt='special'><input type=\"text\" name=\"$field_name\" value=\"".str_replace('"','&quot;',$field_value)."\" style=\"width:100px\" class=\"bando\" readonly>
					 </td>";		
				}
				else{				
					...
					echo"<img src='$style_url/$theme/pile.gif' border='none'><span>Ce champ est relié au tableau <b>$refiled</b></span></a></td><td>
					 <img src='$style_url/$theme/mysqltype-special.png' alt='special'>";
				   if((sizeof($fieldoptions)==3 && $fieldoptions[2]=='ch') || (sizeof($fieldoptions)==2 && $fieldoptions[1]=='nlch')){
